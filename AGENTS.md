...
## CI pipeline
`.github/workflows/ci.yml` runs on every push to any branch. On non-master branches it auto-creates a PR from the pushed branch to `master` and runs Playwright E2E tests. On push to `master` it runs `release-please` to create a release. Does **not** run frontend unit tests, lint, or TypeSpec compilation.
