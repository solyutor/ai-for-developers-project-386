...
make e2e              # run all E2E tests (headless, auto-starts Vite)
...
E2E tests auto-start Vite (port 5173). The API backend must be running separately: `make prism` or `make backend`.

Playwright MCP server is also registered in `.opencode/mcp.json` — opencode can use browser tools (`browser_navigate`, `browser_click`, etc.) during sessions.
