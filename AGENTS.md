...
- `backend/CalendarBooking.Api/Program.cs` — .NET minimal API routes (mirrors TypeSpec contract)
...
## CI pipeline
`.github/workflows/ci.yml` runs only Playwright E2E tests (on push/PR to master). It builds the .NET backend and installs frontend/E2E deps but does **not** run frontend unit tests, lint, or TypeSpec compilation.
