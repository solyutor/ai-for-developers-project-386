...
### Full dev workflow
Or via Makefile (runs from repo root):
```bash
...
- `tsp-output/` is gitignored. After a fresh clone, run TypeSpec compile before starting Prism.
- Prism is a devDependency in `typespec/package.json` — run from `typespec/` via `npx prism mock ...`.
- `OWNER_ID` in `frontend/src/config.ts` is hardcoded — there is no auth/registration system.
- Tests: `npm run test` from `frontend/` runs vitest. Test setup mocks Mantine's `matchMedia` and `ResizeObserver`.
